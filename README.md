...
[![Bug Bounty on Immunefi](https://img.shields.io/badge/bug%20bounty-immunefi-red)](https://immunefi.com)
[![Responsible Disclosure](https://img.shields.io/badge/disclosure-responsible-green)](#security)
...
This project is part of our security-first approach to protecting Solana smart contracts. We welcome responsible security disclosures and have integrated with the Immunefi platform for coordinated vulnerability management.

### Bug Bounty Program

We are committed to security and appreciate the work of security researchers. Visit our [Immunefi page](https://immunefi.com) to report vulnerabilities and potentially earn bug bounties.

For immediate security concerns, email: [rvbruno19@icloud.com](mailto:rvbruno19@icloud.com)
...
In general, there are two ways to specify the information. Either directly inside the contract to store it on-chain or by linking to a web page. The former has the advantage that it is easy to set[...]
...
As many projects are best reachable via Telegram or Discord there is native support for these contact methods. But be aware that handles might change, for example, if you change your Discord usern[...]

The `security_txt` macro is intentionally kept brief. As such, it doesn't do any input validation. For optimal experience, **please verify the format before uploading the contract to the chain.** [...]
...
Bug bounty policies can be a bit daunting to write. For a good and thorough example, take a look at Solana Foundation's [SECURITY.md](https://github.com/solana-labs/solana/blob/master/SECURITY.md[...]

> We pay a bug bounty at our discretion after verifying the bug, up to 10% of value at risk, limited by a maximum of $x. This bounty is only paid out if details about the security issues have not[...]
...
This crate uses a macro to construct one long security.txt string. It begins with the start marker `=======BEGIN SECURITY.TXT V1=======\0`, and ends with the end marker `=======END SECURITY.TXT V[...]
...
| **`contacts`**        |   list (required)    | A comma-separated list of contact information in the format `<contact type>:<contact information>`. Should roughly be ordered in preference. Possi[...]
| **`policy`**          | link/text (required) | Either a link or a text document describing the project's security policy. This should describe what kind of bounties your project offers and the [...]
...
| `source_release`      |   string (optional   | The release identifier of this build, ideally corresponding to a tag on git that can be rebuilt to reproduce the same binary. 3rd party build veri[...]
| `source_revision`     |  string (optional)   | The revision identifier of this build, usually a git sha that can be rebuilt to reproduce the same binary. 3rd party build verification tools will[...]
| `auditors`            | link/list (optional) | A comma-separated list of people or entities that audited this smart contract, or a link to a page where audit reports are hosted. Note that this [...]
...
In addition to inserting the security.txt string into the binary, the macro creates a new `.security.txt` ELF section via the `#[link_section]` attribute. Because of how Rust strings work, it is [...]
